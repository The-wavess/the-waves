			...
			<div class="ding_body_div">
				...
				<div class="ding_right">
					...
					<ul id="input">
						...
						<input v-model="password" maxlength="6"  placeholder="填写验证码" />
						<span id="yanzheng">获取验证码</span>
					</ul>
					<ul>
						<button type="button" id="btn" @click="shopping">确认选座</button>
					</ul>
				</div>
			</div>
		</div>
  ...
  methods: {
    ...
    go() {
      ...
      data0.forEach((element) => {
        let move_name = element.move_name;
        let move_img = element.move_img;
        let price = 37.9;
        let count = this.zuo.length;
        let times = new Date();
        let today = times.toLocaleString();

        this.axios
          .post(
            "/move_insert","move_name=" +
              move_name +
              "&move_img=" +
              move_img +
              "&price=" +
    ...
    ding(e, i) {
      ...
      if (
        this.zuo.length > 0 &&
        img.src == "http://localhost:8080/move_img/zuo_0.png"
      ) {
        this.zuo.pop();
      }
